...
# columns we ALWAYS try to use first
BASE = ["area","perimeter","circularity","aspect_ratio","solidity"]
# optional robust features (use if present in CSV)
...
def main(args):
    ...
    if args.csvdir:
        print(f"Loading all CSVs from folder: {args.csvdir}")
        df = load_many(args.csvdir, args.pattern)
    else:
        print(f"Loading single CSV: {args.csv}")
    ...
    for name, pipe in pipes.items():
        ...
        gs = GridSearchCV(
            ...
            n_jobs=4, refit=True, verbose=2, error_score='raise', pre_dispatch='1*n_jobs'
        )
    ...
    # Optional: refit SVC with probability if needed
